...
{
  string xStr = string(iToken[0], iTokenLen[0]);
  string yStr = string(iToken[1], iTokenLen[1]);

  double xFlt = atof(xStr.c_str());
  double yFlt = atof(yStr.c_str());
  
  string gridId = map_cartesian_to_grid_id(xFlt, yFlt);
