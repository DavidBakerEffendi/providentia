...
\begin{figure*}[h]
    ...
    \end{mdframed}
    \caption{A subset of the graph produced by TigerGraph on the result of the query for \edt{the third kernel, \emph{Ranking Las Vegas by Friends’ Sentiment}}. Orange edges represent friend relations and maroon edges represent reviews. Blue vertices represent users and brown vertices represent businesses. The white center of the cluster on the top right is Julie and once can see the center of the giant cluster on the left is a mutual friend of Julie's. \todo{Compare this with a different user to show how the graph patterns are similar.}}
    \label{fig:cityGraph}
\end{figure*}
