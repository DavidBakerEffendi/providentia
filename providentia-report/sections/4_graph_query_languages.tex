The following section explores the three popular graph querying languages -- two of which are used in the databases being investigated in this report -- and properties and techniques of graph querying. Graph query languages have a notion of traversing the graph and accumulating results from pattern matching. This makes it very different to traditional SQL and is the main factor in contributing to the learning curve -- how to conceptualize graph expressions. These languages are still fairly new \edt{with some of the latest additions being GraphQL by Facebook (released in 2015) \cite{graphqlhome} and GSQL by TigerGraph (released in 2017) \cite{tgv1docs}}. As the technique of graph traversal becomes more refined \edt{ one can expect a few more languages to develop, especially in the later generations of the graph technology evolution.}
...
\edt{It should be noted that the fact that native Cypher is not Turing-complete be a problem dependent consideration. For most ordinary querying, not having a Turing-complete query language poses few to no major drawbacks at all. Examples of algorithms, popularly used on an enterprise level, which cannot be implemented in native Cypher are in PageRank/Label Propagation style algorithms \cite{modern-graph-query-lang}. The workaround is that Cypher provides calls an algorithms library to conduct these. Many important algorithms affected by this limitation are most likely implemented in the Neo4j Graph Algorithms library\footnote{\url{https://neo4j.com/docs/graph-algorithms/3.5/labs-algorithms/}}. An example of this can be seen in Figure \ref{lst:cypher-pagerank}. A problem with a hard-coded PageRank-style algorithm comes with the limitation of not having full control over the number of iterations and terminating conditions. Ultimately, one should check the documentation for this before considering opting for Turing-complete graph query languages.}
